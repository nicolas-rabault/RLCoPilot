#!/usr/bin/env bash
# Launch training on a direct SSH host with GPU check and screen.
...
# Derive project name from remote dir for worktree/screen naming
PROJECT_NAME="$(basename "$REMOTE_DIR")"
SCREEN_NAME="$PROJECT_NAME-$BRANCH_SANITIZED"
WORKTREE_DIR="${REMOTE_DIR%/*}/$PROJECT_NAME-wt-$BRANCH_SANITIZED"
